...
def classification_scores(model, dloader, device, task,vision_dset, logit_adjustment):
    model.eval()
    m = nn.Softmax(dim=1)
    y_test = torch.empty(0).to(device)
    y_pred = torch.empty(0).to(device)
    ...
    with torch.no_grad():
        for i, data in enumerate(dloader, 0):
            ...
            # y_outs = y_outs - logit_adjustment
    ...
    if task == 'binary':
        # auc = roc_auc_score(y_score=prob.cpu(), y_true=y_test.cpu())
        auc_list = roc_curve(y_test.cpu(),prob.cpu())
    return acc.cpu().numpy(), y_pred.cpu(),prob.cpu(),y_test.cpu()
